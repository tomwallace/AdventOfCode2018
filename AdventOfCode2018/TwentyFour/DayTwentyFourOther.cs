...
{
    public class DayTwentyFourOther
    {
        ...
        public FightResult Fight(string input, int b)
        {
            ...
            {
                attack = false;
                var remainingTarget = new HashSet<GroupTwo>(army);
                var targets = new Dictionary<GroupTwo, GroupTwo>();
            ...
            {
                immuneSystem = army.All(x => x.immuneSystem),
                units = army.Select(x => x.units).Sum()
            };
        }
